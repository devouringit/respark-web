...
import { BANNER_NO_IMAGE, SMALL_SLIDER_NO_IMAGE, LARGE_SLIDER_NO_IMAGE, SKILLED_STAFF_NO_IMAGE, SUB_CAT_NO_IMAGE } from '@constant/noImage';
...
import { updateStoreData, updateGroupStatus, showSuccess, updateStore, updateGenericImages } from "app/redux/actions";
...
function HomePage({ storeData, activeGroup, storeMetaData }) {
  ...
  const cartAppointment = useSelector(state => state.appointment.appointmentServices);
  ...
  const changeGroupWiseTheme = (activeGroup) => {
    ...
    let color = configData?.storeConfig?.sparkConfig?.color[activeGroup] || '#ff0185';
  ...
  return (
    <>
      ...
      {(configData.storeConfig?.appointmentConfig?.active && currentPage != 'cart' && currentPage != 'checkout' && currentPage != 'appointment' && !cartAppointment.length) ? <Link href={baseRouteUrl + 'appointment'} shallow={true}>

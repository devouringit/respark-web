...
function Appointment() {
    ...
    const renderCategoryItemsView = (category: any) => {
        if (category && category.display) {
            return <div className={`items-cat-wrap clearfix ${category?.hasSubcategory ? 'has-sub-category' : ''}`}>
                <div className={`cat-name ${category?.hasSubcategory ? 'has-sub-category' : ''}`}>
                    {category.name}
                </div>
                {(category?.hasSubcategory && category?.categoryList?.length != 0) ? <>
        ...
        <>
            {!showAppointmentSuccess ?
                <div className="main-wrapper appointment-wrap">
                    ...
                    <div className="selected-service-list-wrap card">
                                ...
                                >
                                    ...
                                    {categoriesList.length != 0 && <div className="add-services-wrap">
                                        ...
                                        {/* {Object.keys(filteredList).map((key, i) => (
                                            <div className="items-cat-wrap clearfix" key={Math.random()}>
                                                <div className="cat-name"> {key}</div>
                                                {filteredList[key]?.map((item, i) => {
                                                    return <React.Fragment key={Math.random()}>
                                                        <Item item={item} config={{ redirection: false, onClickAction: true }} />
                                                    </React.Fragment>
                                                })}
                    ...
                    {appointmentOrder && <div className="main-wrapper appointment-wrap" style={{ background: 'transparent' }}>
                        <div className='appointment-checkout-content'>
                            ...
                            <div className='confirm-btn-wrap book-wrap alignment'>
                                <button className="primary-btn rounded-btn" onClick={() => router.push({ pathname: baseRouteUrl + 'home' }, '', { shallow: true })}>Explore More</button>
                            </div>
                        </div>
                    </div>}
                </>
